...
export function getKindLabel(kind: NotificationKind) {
    switch (kind) {
        case "voice-join": return "Voice join";
...
export function getKindTone(kind: NotificationKind) {
    switch (kind) {
        case "voice-join": return "success";
        case "voice-leave": return "danger";
...
export function getPlatformColor(platform: string) {
    switch (platform) {
        case "desktop": return "#5865f2";
        case "web": return "#2ea043";
        case "mobile": return "#faa61a";
        case "embedded":
        case "console": return "#f97316";
        default: return "#747f8d";
    }
}
...
export async function recordGameEntry(
    ...
    const activitySummary = activity ? getActivitySummary(activity) : previousActivity ? getActivitySummary(previousActivity) : getActivitySummary(storedActivity);
    ...
    void persistHistoryEntry({
        ...
        body: activity ? activitySummary : `Stopped playing ${previousActivity ? getActivitySummary(previousActivity) : storedActivity.name}`,
        color: "#5865f2",
        avatarUrl,
        previous: previousActivity ? getActivitySummary(previousActivity) : activity ? null : activitySummary,
